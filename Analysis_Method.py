...
def LOAD(filepath,filename,plot=False):
      """
      inputs:
      filepath: str, path where the .txt file is stored
      filename: str, name of the file
      plot: bool
      ---
      outputs:
      XC: list, x-coordinates of the profiles (len(XC) = number of profiles)
      ZC: list, z-coordinates of the profiles (len(ZC) = number of profiles)
      """
...
def PANEL(XC,ZC,M=100,plot=False):
      ...
      XC: list, x-coordinates of the profiles (len(XC) = number of profiles)
      ZC: list, z-coordinates of the profiles (len(ZC) = number of profiles)
      M: int, number of panels (for the integration)
      plot: bool, to plot or not the profiles
      ---
      outputs:
      XP: list, x-coordinates of the panels (len(XP) = number of profiles)
      ZP: list, z-coordinates of the panels (len(XP) = number of profiles)
      """
      K=len(XC)
      N = M+2
      ...
      for k in range(K):
            L[k] = np.where(XC[k] == XC[k].min())[0][0] # index of minimum
...
def ASOLVE(XP,ZP,ALPHA=0,UINF=1,GE=False,h=0.2):
      ...
      ---
      outputs:
      GAMMAK: array of arrays, circulation (len(GAMMAK) = number of profiles)
      PSI: array of floats, streamfunction (len(PSI) = number of profiles)
      CL: array of floats, lift coefficient (len(CL) = number of profiles)
      """
