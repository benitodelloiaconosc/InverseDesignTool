...
def ISOLVE(XP,ZP,GAMMAI,PSII,GAMMASP,XSP,iS,iE,ALPHA=0,UINF=1,GE=False,h=0.2):
      """
      inputs:
            - XP: list of list, x-coordinates of the panels (len(XP) = number of profiles)
            - ZP: list of list, z-coordinates of the panels (len(XP) = number of profiles)
            - GAMMAI: list of list, initial gamma's (len(GAMMA) = number of profiles)
            - PSII: list of list, initial Psi's (len(PSII) = number of profiles)
            - GAMMASP: list of list, user specified gamma via Cp distribution
            - XSP: list of list, user specified x-coordinates linked to GAMMASP
            - iS: list, boundary point index at start of design segment
            - iE: list, boundary point index at end of design segment
            - ALPHA: float, angle of attack
            - UINF: float, entry velocity
            - GE: bool, ground effect
            - h: float, height from ground
      outputs:
            - XNEW: list, x-coordinates of the new profiles
            - ZNEW: list, z-coordinates of the new profiles
            - GAMMANEW: list, new circulation
            - PSISOL: ???, Psi solution
            - CLNEW: list, new lift coefficient / profile
            - [XSOL,ZSOL,GAMMASOL,QSPSOL,ASOL,BSOL]
            - [GAMMASP,GAMMAI]
            - [RES,RESTIME,RTIME,RCOUNT,JACTIME,JACCOUNT]
      """
      # SOLVE THE INVERSE PROBLEM
      # INITIATE ARRAYS
      ALPHA=ALPHA*m.pi/180 # put the angle from deg to rad
      K = len(XP) # number of points
      M = len(XP[0])-1 # length of the first element of the XP coordinates
      ...
      if(GE==True): # if there is ground effect
...
#     GAMMASPK=[0 for k in range(K)]
      ...
      def R(W):
            ...
            #REPLACE OLD COORDINATES WITH NEW GUESS
            for k in range(K):
      ...
      def JAC(W):
            ...
            """
            #SPLIT W INTO X, Z, GAMMA, PSI, A2/A2 ARRAYS
            X=[0 for k in range(K)]
            Z=[0 for k in range(K)]
            GAMMAW=[0 for k in range(K)]
            PSY=[0 for k in range(K)]
