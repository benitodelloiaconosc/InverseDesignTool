...
plt.rc('text', usetex=False) # Imposta usetex=True se hai Latex e vuoi usarlo
...
class MidpointNormalize(mpl.colors.Normalize):
    ...
    def __call__(self, value, clip=None):
        ...
        return np.ma.masked_array(sci.interp(value, x, y))
        # Ho sostituito np.ma a sci.ma perché probabilmente questo metodo era contenuto nella libreria scipy (sci), ma
        # nella versione attuale non c'è. Al contrario è presente nella libreria numpy (np).
...
Plot CP
'''
# Metodo PLOT_CP modificato rispetto al metodo di analisi. Permette di sovrapporre la distribuzione originale e obiettivo
def PLOT_CP(CPX,XP,ZP,CPXNEW,XNEW,ZNEW,GE=False,save=True,savename='test'):
      fig, ax = plt.subplots(nrows=2,ncols=1, figsize=(7,7))   
      for k in range(len(XP)):
          ax[0].plot(XP[k],CPX[k],color='k',linestyle='--',marker='None')
          ax[1].plot(XP[k], ZP[k], color='k', linewidth=1.5)
          ax[0].plot(XNEW[k],CPXNEW[k],color='m',linestyle='None',marker='*')
          ax[1].plot(XNEW[k], ZNEW[k], color='m', linewidth=1.5)
      ...
      ax[0].set_ylabel("Cp",rotation='horizontal') # r"\textbf{C\textsubscript{P}}" se hai latex
      ax[0].set_xlabel("X/C",x=1.05) # r"\textbf{X/C}"
...
def FIELD_PLOT_CP_U_V(U, W, VM, CPF, XG, ZG, XP, ZP, zlimit,h=0,mirror=False,savename='test',save=True):
      ...
      cbar1.set_label("Cp", fontsize=16) # r"$\textrm{C\textsubscript{P}}$" se hai latex
      ax[0].axis('scaled') # Ho cancellato adjustable='box' perché l'attributo adjustable sembrava inesistente.
      if h!=0:
      ...
      cbar2.set_label("u/U_inf", fontsize=16) # r"$\displaystyle |\mathbf{u}|/U_\infty$" se hai latex
      ...
      ax[1].axis('scaled') # Ho cancellato adjustable='box'
      ax[0].set(ylim=(zlimit[0],zlimit[1]))
      ax[1].set(ylim=(zlimit[0],zlimit[1]))
      
      
      ax[0].set_ylabel("Z/C") # r"\textbf{Z/C}" se hai latex
      ax[1].set_ylabel("Z/C") # r"\textbf{Z/C}"
      ax[1].set_xlabel("X/C") # r"\textbf{X/C}"
...
#                               marker='None', linewidth=1.5, zorder=3,alpha=0.8)
      ...
      ax.axis('scaled') # Ho cancellato adjustable='box'
      ax.set(xlim=(-0.5,1.5))
      ax.set(ylim=(-0.1,1.0))
      
      ax.set_ylabel("Z/C") # r"\textbf{Z/C}"
      ax.set_xlabel("X/C") # r"\textbf{X/C}"
...
def FIELD_PLOT_POTENTIAL(PHI, XG, ZG, XP, ZP, h=0,mirror=False):
      ...
      cax1 = divider1.append_axes("right", size="5%", pad=0.1)
      cbar1 = plt.colorbar(cpfplot, cax=cax1)
      cbar1.set_label("Phi", fontsize=16,rotation=90) # r"$\Phi$"
